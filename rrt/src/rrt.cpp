...
RRT::RRT(ros::NodeHandle &nh): nh_(nh), gen((std::random_device())()) {
    ...
    local_viz_pub = nh_.advertise<visualization_msgs::Marker>( "visualization_marker3", 10);
...
//void RRT::pf_callback(const geometry_msgs::PoseStamped::ConstPtr &pose_msg) {
    ...
    //if(path.size() > 0){
        ...
        //double robot_x = pose_msg->pose.position.x;
        //double robot_y = pose_msg->pose.position.y;
        //double candidate_x;
        //double candidate_y;
        //std::vector<double> candidate_x_robot; // used to store all the candidates
        //std::vector<double> candidate_y_robot;
        ...
        /*for(int i = 0; i < path.size(); i++){
            ...
            if(d < local_lookhead ){
                // get transform for candidate path point
                geometry_msgs::PointStamped point_before;
                point_before.point.x = path_point.x;
                point_before.point.y = path_point.y;
                point_before.header.frame_id = "/map";
                geometry_msgs::PointStamped point_after;
                point_after.header.frame_id = "/laser";
                listener.transformPoint("/laser",point_before,point_after);
                // pick one candidate waypoint
                double waypoint_x_robot = point_after.point.x;
                double waypoint_y_robot = point_after.point.y;
                double angle = atan2(waypoint_y_robot,waypoint_x_robot);
                if(abs(angle) < (M_PI/2.0)){
                    candidate_x = waypoint_x_robot;
                    candidate_y = waypoint_y_robot;
                    break;
                }
            }
        }*/
        ...
        double steering = 0.8*2*best_candidate_y/(pow(local_lookhead,2.0));
...
/*void RRT::exeu_callback(const geometry_msgs::PoseStamped::ConstPtr& pose_msg){
    // execuate the path trajectory for each new pose update
    double local_lookhead = 1.0;
    double robot_x = pose_msg->pose.position.x;
    double robot_y = pose_msg->pose.position.y;
    double candidate_x;
    ...
    for(int i = 0; i < path.size(); i++){
        Node path_point = path[i];
        double d = sqrt(pow(path_point.x - robot_x,2.0)+pow(path_point.y - robot_y,2.0));
        if(d > local_lookhead && d < (local_lookhead + 0.2)){
            // get transform for candidate path point
            geometry_msgs::PointStamped point_before;
            point_before.point.x = path_point.x;
            point_before.point.y = path_point.y;
    ...
    if(abs(candidate_y) < 0.05){
         candidate_y = 0.0;
    }

    double steering = 0.8*2*candidate_y/(pow(local_lookhead,2.0));
    if(steering <= -0.3189){
        steering = -0.3189;
    }
    if(steering >= 0.3189){
